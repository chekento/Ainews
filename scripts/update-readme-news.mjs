...
out=out.replace(/DOWNLOAD_ANDROID_[0-9.]+(?:_BETA)?/g,'DOWNLOAD_ANDROID_3.2_BETA')
  .replace(/CURRENT APK — ANDROID [0-9.]+ BETA/g,'CURRENT APK — ANDROID 3.2 BETA')
  .replace(/### 📱 ANDROID [0-9.]+ BETA/g,'### 📱 ANDROID 3.2 BETA')
  .replace(/## 📱 Android [0-9.]+[^\n]*/g,'## 📱 Android 3.2 Beta — Discover & Expanded Intelligence')
  .replace(/AI News Android [0-9.]+(?: Beta)?/g,'AI News Android 3.2 Beta')
  .replace(/Android \*\*[0-9.]+\*\*/g,'Android **3.2**')
  ...
  .replace(/badge\/\d+_LLM_ECOSYSTEMS-/g,`badge/${data.providerCount||0}_AI_PROVIDER_ECOSYSTEMS-`)
  .replace(/alt="\d+ LLM providers"/g,`alt="${data.providerCount||0} AI providers"`);
...
console.log(`README refreshed with ${items.length} stories, ${data.sourceCount} sources, ${data.providerCount} providers and Android 3.2 Beta.`);
